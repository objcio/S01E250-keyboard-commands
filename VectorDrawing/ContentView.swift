    ...
    var body: some View {
        if let cp = element.controlPoints, drawControlPoints {
            ...
            controlPoint(at: cp.0) { p, option in
                element.moveControlPoint1(to: p, option: option)
            }
            controlPoint(at: cp.1) { p, option in
                element.moveControlPoint2(to: p, option: option)
            }
        }
...
struct Points: View {
    ...
    var body: some View {
        ...
        ForEach(Array(zip(drawing.elements, drawing.elements.indices)), id: \.0.id) { pair in
            ...
            PathPoint(element: $drawing.elements[pair.1], selected: selected, drawControlPoints: drawControlPoints, select: { exclusive in
